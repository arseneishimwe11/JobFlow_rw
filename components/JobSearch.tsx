...
export default function JobSearch() {
  ...
  return (
    <div className={`p-6 rounded-3xl backdrop-blur-xl border shadow-2xl mb-8 ${theme === 'dark'
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
          ...
          <Input
            ...
            onKeyDown={(e) => {
              if (e.key === 'Enter') {
                ...
                e.preventDefault();
                // The filter is already updated on change, so we don't need to do anything else
              }
            }}
            className={`pl-12 h-14 text-lg rounded-2xl border-0 ${theme === 'dark'
        ...
        <div className="relative md:w-64">
          <MapPin className={`absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'
            }`} />
          <Input
            placeholder="Location"
            value={filters.location || ''}
            onChange={(e) => updateFilter('location', e.target.value)}
            className={`pl-12 h-14 rounded-2xl border-0 ${theme === 'dark'
                ? 'bg-white/10 text-white placeholder:text-gray-400'
                : 'bg-white/80 text-gray-900 placeholder:text-gray-500'
              } backdrop-blur-sm`}
          />
        ...
        <Button
          ...
          variant="outline"
          className={`h-14 px-6 rounded-2xl border-0 ${theme === 'dark'
      ...
      {showAdvanced && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-6 border-t border-white/10">
          {/* Category Filter - Commented out until backend supports it
          ...
          */}
          ...
          <Select value={filters.jobType || ''} onValueChange={(value) => updateFilter('jobType', value)}>
            <SelectTrigger className={`h-12 rounded-xl border-0 ${theme === 'dark'
                ? 'bg-white/10 text-white'
                : 'bg-white/80 text-gray-900'
              } backdrop-blur-sm`}>
