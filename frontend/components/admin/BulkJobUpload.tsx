  ...
  const sampleTemplate = JSON.stringify([
    {
      ...
      deadline: "2024-12-31",
      source: "Manual",
      url: "https://techsolutions.rw/careers/react-developer"
    },
    {
      title: "Marketing Specialist",
      company: "Digital Marketing Agency",
      location: "Kigali, Rwanda",
      description: "Seeking a creative marketing specialist to develop and execute marketing campaigns...",
      requirements: "2+ years marketing experience, Social media expertise",
      salaryRange: "400,000 - 600,000 RWF/month",
      jobType: "Full-time",
      category: "Marketing",
      source: "Manual",
      url: "https://digitalagency.rw/careers/marketing-specialist"
    }
  ], null, 2);
  ...
  const validateJsonData = useCallback(() => {
    ...
    try {
      ...
      parsed.forEach((job, index) => {
        ...
        if (!job.description?.trim()) jobErrors.push(`description is required`);
        if (!job.jobType?.trim()) jobErrors.push(`jobType is required`);
        if (!job.category?.trim()) jobErrors.push(`category is required`);
        if (!job.url?.trim()) jobErrors.push(`url is required`);

        // URL validation
        if (job.url && !job.url.startsWith('http')) {
          jobErrors.push(`url must be a valid HTTP/HTTPS URL`);
        }
