...
export default function JobFilters() {
  ...
  const { data: statsData, isLoading: statsLoading } = useQuery({
    queryKey: ['job-filters-data'],
    queryFn: () => apiClient.stats.get(),
  });

  const jobTypes = statsData?.jobTypes?.map(item => item.name) || ['Full-time', 'Part-time', 'Contract', 'Internship', 'Remote'];
  const categories = statsData?.categories?.map(item => item.name) || ['Technology', 'Marketing', 'Finance', 'Healthcare', 'Education', 'Sales'];
  const locations = statsData?.locations?.map(item => item.name) || ['Kigali', 'Butare', 'Gisenyi', 'Ruhengeri', 'Nyanza'];
  const companies = statsData?.topCompanies?.map(item => item.name) || ['Bank of Kigali', 'MTN Rwanda', 'Zipline', 'Andela', 'RDB'];
